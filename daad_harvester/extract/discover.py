...
Integration status: this module is NOT YET wired into daad_harvester.cli. The active
discovery implementation is daad_harvester.discover.Discoverer (1000+ lines, covering
per-site adapters, seed handling, and fixes from the PR #13 forensic audit -- e.g.
previously-fabricated seed URLs and broken discovery endpoints). The adapters below
(SpectrumComputingAdapter, Plus4WorldAdapter, CSDbAdapter, InternetArchiveAdapter) are
thin skeletons that all share the same generic parse_catalog_page with no per-site
logic yet, so wiring this in as-is would be a functional regression, not just an
architecture change. Treat this as a starting point for a future, fixture-verified,
incremental migration of Discoverer's adapters -- one site at a time, each checked
against the existing discovery test fixtures -- not a drop-in replacement.
"""
    ...
    def extract_links_selectolax(html_content: str, base_url: str) -> List[str]:
        ...
        for node in tree.css("a[href]"):
            ...
            if href and not href.startswith(("javascript:", "mailto:", "#")):
                # urljoin correctly handles every case a hand-rolled version tends to
                # miss: absolute URLs, protocol-relative URLs ("//cdn.example.com/x"),
                # root-relative paths, and "../"-style relative paths -- the last of
                # which a naive `base_url.rsplit("/", 1)[0] + "/" + href` never
                # resolves, silently producing malformed URLs with literal ".." segments.
