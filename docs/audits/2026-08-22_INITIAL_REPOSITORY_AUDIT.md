...
| P1 | The root roadmap labels a historical test-count baseline as latest. | The first TODO status baseline declares 409 passing tests. | Refresh it only with a verified current primary-gate output, retaining it as an auditable baseline rather than a mutable assertion. |
