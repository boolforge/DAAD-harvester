...
| **Status** | Implementation contract. No native writer is promoted by this document alone. |
| **Implementation links** | `daad_harvester.media_inspection._inspect_cpc_dsk`, `daad_harvester.unpack.Unpacker.unpack_dsk`, `tests/test_media_inspection.py`, `tests/test_unpack.py`. |
...
The static report, Pages portal, and TUI must render the same status, input/output checksums, generator options, technical-medium role, comparison boundary, and reproducible command. Generated bytes become publicly downloadable only through the manifest-controlled public artifact pipeline.
