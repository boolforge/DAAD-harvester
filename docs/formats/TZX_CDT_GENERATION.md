...
| **Status** | Generation contract. No native TZX/CDT writer is promoted by this document alone. |
| **Implementation links** | `daad_harvester.platform_media.parse_tzx_blocks`, `daad_harvester.platform_media.extract_tzx`, `daad_harvester.media_inspection._inspect_tzx`, `daad_harvester.unpack.Unpacker.unpack_tzx`, `tests/test_platform_media.py`. |
...
When promoted, the writer must join the same canonical `daad_harvester.generator_evidence` collection used by the Extended DSK and ADF/OFS fixtures. The primary workflow must regenerate and pin its output; the static report, Pages `#generators` route, and TUI Native Generators tab must expose its ID, profile, all 17 checksums, validation record, and explicit comparison boundaries. Generated bytes can be published only via the manifest-controlled artifact pipeline after their published copies are checksum-verified.
