...
| Header field | Value |
| --- | --- |
| **Question** | What machine-readable fields make an authorized reverse-engineering artifact reproducible, attributable, and scientifically publishable? |
...
| `authorization` | object | Institutional handling directive plus state, evidence URL/path, scope, issuer/source, decision date, and recorded provenance context. |
| `storage` | object | Immutable repository path and storage-integrity state. |
...
| `publication_status` | Published corpus state; records provenance context without blocking authorized publication. |
...
```json
{
  ...
  "authorization": {"state": "institutional_authorized", "provenance_context": "user_claimed_permission"},
  "storage": {"status": "retained_immutable", "path": "reverse_engineering/artifacts/original/zx/example.bin"},
  "analysis": []
}
```
