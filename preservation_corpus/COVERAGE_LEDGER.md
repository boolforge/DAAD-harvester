...
| --- | --- | --- |
| C64 and Plus/4 game members use load-addressed PRG-like members and loader/runtime packaging rather than a directly validated DDB header. A complete retained-member scan found no valid embedded DDB in `EDI64`, `EDIPLUS4`, `JABATO P.1`, `JABATO P.2`, or their companion data members; the apparent legacy markers in `EDI64`/`EDIPLUS4` fail bounded length/pointer validation and are code coincidences. | [`evidence_commodore_loader_ddb_candidates.json`](evidence_commodore_loader_ddb_candidates.json); 6502 disassemblies under [`derived/commodore_loader/`](derived/commodore_loader/); and the verified [`runtime_resources/commodore/`](runtime_resources/commodore/) manifest. The credits-state Side A RAM image contains no valid DDB, but the original playable Side A state contains a verified legacy V1 C64 DDB at RAM `$3880` (24,899 bytes), materialized as [`derived/commodore_loader/jabato_side_a_tap_second_space_part1.ddb`](derived/commodore_loader/jabato_side_a_tap_second_space_part1.ddb). Its SHA-256 is exactly identical to ADP's retained `JABATO-C64 1.DDB` fixture; the parser now validates both using the documented V1 file-length field at `$001E`. Native raw-TAP support now materializes four parity-valid ROM-reader descendants on `--reunpack-source 248`: duplicate 202-byte `JABATO 1` headers and duplicate 299-byte low-memory-loader packets. These verified bootstrap packets contain no Part 1 DDB prefix. The complete Plus/4 BASIC/KERNAL/3-Plus-1 and hardware-1541 environment now also boots. `EDIPLUS4` has a retained `SYS 2063` bootstrap and an independently verified D64 directory entry, but hardware-emulated execution still reports `FILE NOT FOUND`; logs, snapshots, and screenshots retain this exact interaction. [7] | Extend the physical Side A decoder through the later custom/adaptive payload path until it reproduces the validated Part 1 bytes without RAM extraction; acquire/execute Side B or a corresponding later-load path to recover and verify Part 2; then compare both original parts with the ASS D64 package and resolve the Plus/4 serial/D64 target-media interaction. Keep every remaining loader semantic as a named, reproducible blocker until the full release-variant matrix is verified. |
