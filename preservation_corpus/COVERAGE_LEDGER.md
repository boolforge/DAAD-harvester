...
| --- | --- | --- |
| C64 and Plus/4 game members use load-addressed PRG-like members and loader/runtime packaging rather than a directly validated DDB header. A complete retained-member scan found no valid embedded DDB in `EDI64`, `EDIPLUS4`, `JABATO P.1`, `JABATO P.2`, or their companion data members; the apparent legacy markers in `EDI64`/`EDIPLUS4` fail bounded length/pointer validation and are code coincidences. | [`evidence_commodore_loader_ddb_candidates.json`](evidence_commodore_loader_ddb_candidates.json); 6502 disassemblies under [`derived/commodore_loader/`](derived/commodore_loader/); and the verified [`runtime_resources/commodore/`](runtime_resources/commodore/) manifest. The C64 BASIC/KERNAL pair was built from pinned source and compared byte-for-byte against independent preservation binaries; VICE 3.7.1 then completed virtual-drive autostart of Jabato, reached its title screen, accepted one retained space-input transition, and emitted a snapshot plus exact 64 KiB RAM range. No structurally valid DDB is present in that captured introduction state. The complete Plus/4 BASIC/KERNAL/3-Plus-1 and hardware-1541 environment now also boots. `EDIPLUS4` has a retained `SYS 2063` bootstrap and an independently verified D64 directory entry, but hardware-emulated execution still reports `FILE NOT FOUND`; logs, snapshots, and screenshots retain this exact interaction. [7] | Continue controlled runtime reconstruction through input states, disk-I/O transitions, decompression traces, and game-part boundaries; extract and validate any emitted RAM DDB range; compare memory maps, interpreter behavior, and runtime bytes. Resolve the Plus/4 serial/D64 target-media interaction against the retained `EDIPLUS4` directory and loader bootstrap. Keep every unmodeled loader semantic as a named, reproducible blocker until resolved. |
