...
| 121 | C64 | Jabato | `121_Jabato_1989_Aventuras_AD_es_cr_ASS.d64` | `4f2c17a2324f539dbf0c121b88fb1101602ffed0473481be61800571424c157f` | D64 extracted; both scene members executed and runtime DDBs materialized | 5 | 2 |
...
## Acquisition batch 004 — checksum-pinned Spectrum relationship entries

On 2026-08-22, the source-specific Spectrum Computing discovery adapter identified 17 direct media records whose displayed title, publisher, and year matched retained catalog candidates. The candidate language remains `Unknown` where the relationship catalog omitted it; the observed source language is persisted separately rather than normalized into the catalog claim. Each acquired archive was measured, matched to a subsequently pinned SHA-256 source checksum, copied into the retained corpus, extracted with the native path, and fingerprinted without a replayed network request. The complete per-source root hash, source identifier, artifact count, and structural DDB count are in [`derived/spectrum_observed_acquisition_integration.json`](derived/spectrum_observed_acquisition_integration.json).

> **Boundary:** This batch retains verified source bytes and native extraction results. It does not establish that every represented title, part, language, platform port, release edition, runtime behavior, or unsupported resource family is complete. A structurally recognized DDB remains an artifact-level result, not a semantic equivalence claim for an entire release.
...
| --- | --- | --- |
| C64 and Plus/4 game members use load-addressed PRG-like members and loader/runtime packaging rather than a directly validated DDB header. A complete retained-member scan found no valid embedded DDB in `EDI64`, `EDIPLUS4`, `JABATO P.1`, `JABATO P.2`, or their companion data members; the apparent legacy markers in `EDI64`/`EDIPLUS4` fail bounded length/pointer validation and are code coincidences. | [`evidence_commodore_loader_ddb_candidates.json`](evidence_commodore_loader_ddb_candidates.json); 6502 disassemblies under [`derived/commodore_loader/`](derived/commodore_loader/); and the verified [`runtime_resources/commodore/`](runtime_resources/commodore/) manifest. Corrected V1 embedded-candidate validation shows that both retained Side A credits and gameplay RAM contain the same verified legacy V1 C64 DDB at `$3880` (24,899 bytes), materialized as [`derived/commodore_loader/jabato_side_a_tap_second_space_part1.ddb`](derived/commodore_loader/jabato_side_a_tap_second_space_part1.ddb), byte-identical to ADP's retained `JABATO-C64 1.DDB` fixture. Source 249 retains original Side B; its timed, controlled-input series establishes a first verified Part 2 state at twelve seconds, with a 24,680-byte C64 V1 DDB at `$3880`, materialized as [`derived/commodore_loader/jabato_side_b_tap_post_input_12_part2.ddb`](derived/commodore_loader/jabato_side_b_tap_post_input_12_part2.ddb) and byte-identical to ADP's `JABATO-C64 2.DDB` fixture. Source 121’s retained ASS scene members each reach a post-command runtime state with a structurally verified V1 DDB at `$3880`; its Part 1 and Part 2 ranges are byte-identical to the original Side A and Side B DDBs and are materialized as [`derived/commodore_loader/jabato_ass_part1_post_mirar.ddb`](derived/commodore_loader/jabato_ass_part1_post_mirar.ddb) and [`derived/commodore_loader/jabato_ass_part2_post_mirar.ddb`](derived/commodore_loader/jabato_ass_part2_post_mirar.ddb). Native raw-TAP support materializes duplicate ROM reader headers and low-memory loader packets for both sides; Side A ten-pulse modeling recovers seven 256-byte blocks with exact retained-RAM matches, but neither side is yet fully reconstructed from pulses alone. The complete Plus/4 BASIC/KERNAL/3-Plus-1 and hardware-1541 environment now also boots. `EDIPLUS4` has a retained `SYS 2063` bootstrap and an independently verified D64 directory entry, but hardware-emulated execution still reports `FILE NOT FOUND`; logs, snapshots, and screenshots retain this exact interaction. [7] | Extend the physical Side A and Side B payload decoders until they reproduce all validated Part 1 and Part 2 bytes without RAM extraction, then resolve the Plus/4 serial/D64 target-media interaction. The ASS scene-variant comparison is resolved; keep every remaining loader semantic as a named, reproducible blocker until the full release-variant matrix is verified. |
