...
This comparison exposed and corrected a Harvester V1 parser defect: V1 has eleven section pointers, no extra-object-attributes field, and file length at `$001E`; it is not a 34-byte V2 header with a field omitted semantically. The corrected parser validates both the ADP fixture and the extracted original runtime range as Spanish C64 DAAD legacy V1.

The retained original Side B TAP independently corroborates the second fixture. In a controlled VICE run, the first verified DDB state appears twelve seconds after the title-space input at `$3880`–`$98E7` (24,680 bytes). The materialized range `jabato_side_b_tap_post_input_12_part2.ddb` has SHA-256 `ee3b41f61fc01c27ad776e271f313a2e3fc88875cc9c8e0debebe6904fa48e69` and is byte-identical to ADP’s `JABATO-C64 2.DDB`. The two-part real-artifact regression set runs with the complete suite, which passes 231 tests at this checkpoint.
...
| ADP `JABATO-C64 2.DDB` | `ee3b41f61fc01c27ad776e271f313a2e3fc88875cc9c8e0debebe6904fa48e69` | Exact 24,680-byte match to the first verified Side B RAM range at `$3880`–`$98E7` | **Verified comparator and recovered Part 2 DDB** |
...
The initial integration sequence is complete for both parts: fixture hashing, structural validation, and exact original-runtime correspondence are retained for Side A Part 1 and Side B Part 2. The remaining sequence is to reproduce both ranges through physical TAP decoding and validate the scene-packaged ASS D64 against both original parts. Only exact correspondence or emulator-backed behavioral agreement may promote an ADP-derived claim into Jabato release evidence.
