...
| [z80dismblr](https://github.com/maziac/z80dismblr) | Command-line Z80 disassembler whose README describes CFG-based traversal, optional trace input, labels, comments, call graphs, and warnings for incompletely understood binaries. Revision `2ff52822f0d706f9f39762e079f300f45cfd451d` passed its own 64-test source suite only after an invocation-level TypeScript library workaround. | Pinned, health-checked Z80 comparison candidate. | A trace, start address, SNA, or command-specific label must not be inferred for a raw retained interpreter; any future adapter must retain each supplied assumption. The standard compile command currently conflicts with host dependency declarations, so it is not a workflow dependency. |
...
| [REDasm](https://github.com/redasm-dev/redasm) | README revision `113245ef02090602978e461fcb7031b75da448d6` declares x86/x86_64 and DOS MZ/COM support, an intermediate representation and analysis views, but no decompiler and no Z80 processor entry. Its separate workspace/core documentation confirms that this repository is GUI-oriented and does not supply a stable batch command. | Pinned i8086-family supplemental-analysis candidate, not a Z80 tool. | Do not install as a workflow dependency or configure against retained DOS bytes until a pinned, reproducible headless command plus COM/MZ and segment/load model are independently evidenced. |
