...
from daad_harvester.analyzer_adapters import adapters_for_architecture, load_catalog
...
ADAPTER_CATALOG_PATH = ROOT / "reverse_engineering" / "workflows" / "analyzer_adapters.json"
...
def analyze(
    ...
    for adapter in adapters_for_architecture(adapter_catalog, architecture):
        runner = adapter["runner"]
        if runner == "ghidra_headless_binary" and not include_ghidra:
            continue
        destination = target / adapter["output"]["filename"]
        if runner == "radare2_static":
            r2 = config["radare2"]
            command = [
                workflow["tools"]["radare2"]["command"], "-N", "-a", r2["architecture"], "-b", str(r2["bits"]),
                "-qc", f"aaa;afl;pD {input_size}", str(source),
            ]
        elif runner == "architecture_static":
            command = [*config["independent_static_command"], str(source)]
        elif runner == "ghidra_headless_binary":
            ...
            raise ValueError(f"configured adapter has unsupported runner: {runner}")
        records.append({
            "tool": adapter["record_tool"],
            "adapter_id": adapter["adapter_id"],
            **write_command_output(destination, command),
        })
...
def main() -> int:
    ...
    adapter_catalog = load_catalog(ADAPTER_CATALOG_PATH, workflow)
    ...
    for artifact in artifacts:
        result = analyze(artifact, workflow, adapter_catalog, not args.skip_ghidra)
