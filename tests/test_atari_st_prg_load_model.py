...
def test_retained_atari_prgs_validate_without_execution_promotion():
 c=load_atari_st_prg_load_model(CONTRACT,ROOT); f=parse_atari_st_prg((ROOT/c["profiles"][0]["input_path"]).read_bytes()); assert c["execution_eligible"] is False; assert all(p["launch_capture_observation"] is None for p in c["profiles"]); assert f["first_relocation_offset"]==16; assert f["relocation_stream_size"]==712; assert f["symbol_size"]==0; assert f["relocation_count"]==708
@pytest.mark.parametrize("field,value,msg",[("execution_eligible",True,"must not enable execution"),("sha256","0"*64,"retained Atari ST identity differs"),("data_size",0,"data_size differs"),("symbol_size",1,"symbol_size differs"),("relocation_count",0,"relocation_count differs"),("launch_capture_observation",{"machine_configuration":"generic"},"no official Atari ST launch capture")])
