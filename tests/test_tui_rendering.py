...
def test_render_native_generator_tab_exposes_complete_checksum_and_boundary(tmp_path):
    ...
    assert "3 of 4" in tzx_text
    assert "tzx-standard-data-synthetic-pair-v1" in tzx_text
    assert "generated_extraction_verified" in tzx_text

    dash.selected_index = 54
    chr_text = _render(dash, tabs=(3,), width=180)
    assert "4 of 4" in chr_text
    assert "pcw-torreoscura-validated-chr-atlas-v1" in chr_text
    assert "byte_index_only_no_code_page_claim" in chr_text
