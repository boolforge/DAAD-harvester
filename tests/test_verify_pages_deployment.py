...
def _manifest_and_artifact() -> tuple[bytes, bytes]:
    artifact = b"staged-retained-artifact"
    manifest = json.dumps(
        {
            "artifact_count": 1,
            "artifacts": [
                {
                    "public_path": "artifacts/000001_fixture.tap",
                    "checksums": {"sha256": hashlib.sha256(artifact).hexdigest()},
                }
            ],
        }
    ).encode("utf-8")
...
def test_verify_once_accepts_measured_matching_report_and_documentation() -> None:
    ...
    manifest, artifact = _manifest_and_artifact()
    with patch.object(
        verify_pages_deployment,
        "fetch",
        side_effect=[
            b"<title>DAAD Harvester</title>", report, manifest, artifact,
            b"<h1>Documentation index</h1>Clone-to-Contribution",
        ],
    ):
