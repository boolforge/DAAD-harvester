...
type Section = "overview" | "ledger" | "games" | "resources" | "sources" | "generators" | "detections" | "library" | "logs";
...
const SECTIONS: Array<{ id: Section; label: string; icon: typeof Archive }> = [
  ...
  { id: "resources", label: "Resource evidence", icon: FolderArchive },
...
function resourceKind(artifact: Artifact) {
  const extension = filenameExtension(artifact.original_filename);
  if (artifact.is_daad_payload) return "Game database";
  if (artifact.media_parser === "daad-legacy-chr" || [".chr", ".ch0", ".fnt"].includes(extension)) return "Character set";
  if (artifact.media_parser === "daad-pcw-dat-v1" || extension === ".dat") return "Resource table";
  if (extension === ".pic") return "Picture candidate";
  if (artifact.media_parser) return "Validated technical medium";
  if ([".tap", ".tzx", ".cdt", ".d64", ".dsk", ".adf", ".cas"].includes(extension)) return "Technical medium";
  return "Retained companion byte";
}
function resourceBoundary(artifact: Artifact) {
  if (artifact.media_parser === "daad-pcw-dat-v1") return "Native PCW DAT directory and image-resource decoding evidence is recorded; this export does not stage a preview derivative yet.";
  if (artifact.media_parser === "daad-legacy-chr") return "CHR container and any recognized header profile are measured. A profile-validated glyph atlas is recorded in Native generators; a direct accessible per-artifact preview view remains pending.";
...
export default function Home() {
  ...
  const resourceArtifacts = useMemo(
    () => filteredArtifacts.filter((artifact) => artifact.is_daad_payload || artifact.media_parser || [".chr", ".ch0", ".fnt", ".dat", ".pic", ".tap", ".tzx", ".cdt", ".d64", ".dsk", ".adf", ".cas"].includes(filenameExtension(artifact.original_filename))),
    [filteredArtifacts],
  );
  const resourceSummary = useMemo(() => {
    const summary = new Map<string, number>();
    resourceArtifacts.forEach((artifact) => summary.set(resourceKind(artifact), (summary.get(resourceKind(artifact)) || 0) + 1));
  ...
  else if (route.section === "resources") content = <section className="resource-view"><div className="view-heading"><div><p className="eyebrow">BUNDLE MEMBER → NATIVE EVIDENCE → PUBLISHED BOUNDARY</p><h2>Resource and analysis evidence</h2><p>Resources are grouped by measured file role, not by filename assumption. This view lists the exact retained bytes that can support future verified image, audio, font, decompilation, or interpreter-analysis publication.</p></div>{filterControls}</div><div className="resource-summary" aria-label="Resource evidence categories">{resourceSummary.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong><small>matching retained records</small></article>)}</div><div className="resource-notice"><CircleAlert size={17} /><span>No per-artifact preview, playback, or decompilation is shown until an accessible profile-specific derivative view and its checksum are exported. Validated atlas evidence, when present, is disclosed in Native generators; open any resource record to inspect its original byte and remaining boundary.</span></div><div className="resource-table-wrap"><table><thead><tr><th>Resource role</th><th>Artifact</th><th>Native evidence</th><th>Publication boundary</th><th>Detail</th></tr></thead><tbody>{resourceArtifacts.slice(0, 120).map((artifact) => <tr key={artifact.id}><td><span className="resource-kind">{resourceKind(artifact)}</span></td><td><a className="entity-link" href={routeHash({ section: "resources", entity: "artifact", id: String(artifact.id) })}><strong>{artifact.original_filename}</strong><span>{platformOf(artifact)} · {bytes(artifact.file_size)} · {artifact.sha256.slice(0, 16)}…</span></a></td><td>{artifact.media_parser ? <><code>{artifact.media_parser}</code><span>{artifact.media_validation || "native record"}</span></> : <span>No promoted native parser</span>}</td><td><small className="resource-boundary-cell">{resourceBoundary(artifact)}</small></td><td><a className="inspect" href={routeHash({ section: "resources", entity: "artifact", id: String(artifact.id) })}>Open record</a></td></tr>)}</tbody></table>{resourceArtifacts.length > 120 && <p className="table-note">Showing first 120 matching resource records. Narrow the URL-persisted filter to continue.</p>}</div></section>;
